...
$(document).ready(function() {
    ...
    window.adsrGraph = new Graph(initialData);
    $("#adsr svg").hide();

    $("#adsr-use-graph").on("change", function() {
        if ($("#adsr-use-graph").prop("checked")) {
            $("#adsr-use-multistage").prop("disabled", false);
            $("#adsr svg").show();
            $("#adsr-attack-duration").prop("disabled", true);
            $("#adsr-hold-duration").prop("disabled", true);
            $("#adsr-decay-duration").prop("disabled", true);
            $("#adsr-release-duration").prop("disabled", true);
            $("#adsr-sustain-level").prop("disabled", true);
            $("#adsr-hold-level").prop("disabled", true);
        } else { 
            $("#adsr-use-multistage").prop("disabled", true);
            $("#adsr-multistage-points").prop("disabled", true);
            $("#adsr svg").hide(); 
            $("#adsr-attack-duration").prop("disabled", false);
            $("#adsr-hold-duration").prop("disabled", false);
            $("#adsr-decay-duration").prop("disabled", false);
            $("#adsr-release-duration").prop("disabled", false);
            $("#adsr-sustain-level").prop("disabled", false);
            $("#adsr-hold-level").prop("disabled", false);
        }
    });

    $("#adsr-use-multistage").prop("disabled", true);
    $("#adsr-multistage-points").prop("disabled", true);
    $("#adsr-multistage-points").on("change", function () {
        let value = parseInt($("#adsr-multistage-points").val());

        if (value < 6) $("#adsr-multistage-points").val(6)
        else if (value > 20) $("#adsr-multistage-points").val(20)

        window.adsrGraph.switchMode()
    });
    $("#adsr-use-multistage").on("change", function () {
        window.adsrGraph.switchMode();
    });
});
